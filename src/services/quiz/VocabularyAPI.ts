...
import { GET_HINT_PATH, GET_QUIZ_DATA_PATH } from '@constants/constant'
...
async function getVocabularyHint(
  ...
  const path = `${GET_HINT_PATH}/Vocabulary?studentHistoryId=${studentHistoryId}&studyId=${studyId}&quizNo=${quizNo}`
...
async function getVocabularyPractice4(
  ...
  const transformObject = async (raw: any): Promise<IVocabulary4Practice> => {
    return {
      ContentsId: Number(raw.ContentsId),
      IsQuizTimeoutIncorrect: Boolean(raw.isQuizTimeoutIncorrect),
      QuizAnswerCount: Number(raw.QuizAnswerCount),
...
async function getVocabularyTest4(
  ...
  const transformObject = async (raw: any): Promise<IVocabulary4Test> => {
    return {
      ContentsId: Number(raw.ContentsId),
      IsQuizTimeoutIncorrect: Boolean(raw.isQuizTimeoutIncorrect),
      QuizAnswerCount: Number(raw.QuizAnswerCount),
